  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-6 space-y-8 pt-12">
      ...
      <div className="space-y-4">
        ...
        <button onClick={() => onNavigate('manage')} className="w-full bg-white text-black border border-black/10 p-6 rounded-[2rem] shadow-sm hover:border-black transition-all flex flex-col items-center space-y-3 active:scale-[0.98]">
          <Clock size={40} />
      ...
      {myBookings.length === 0 ? (
        ...
        <div className="space-y-4">
          {myBookings.map(booking => {
            ...
            if (!room) return null;
            return <BookingCard key={booking.id} booking={booking} room={room} />;
          })}
        </div>
      )}
    </motion.div>
...
function BookingCard({ booking, room }: { booking: Booking, room: Room }) {
  ...
  return (
    <div className="bg-white p-6 rounded-3xl border border-black/10 shadow-sm space-y-4">
      <div className="flex justify-between items-start">
        ...
        <span className={`px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider ${isActive ? 'bg-black text-white' : 'bg-black/5 text-black'}`}>
      ...
      <div className="text-sm font-medium text-gray-500 space-y-1 border-t border-black/5 pt-4">
        <p>Llegada: {new Date(booking.checkInDate).toLocaleString('es-BO')}</p>
...
function NewReservationFlow({ onFinish }: { onFinish: () => void }) {
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 3: return <PaymentSelection data={reservation} updateData={updateData} onNext={handleConfirm} />;
      case 4: return <Confirmation onFinish={onFinish} />;
      default: return null;
    }
  };
...
function Confirmation({ onFinish }: { onFinish: () => void }) {
  return (
    <div className="px-5 pt-20 pb-24 flex flex-col items-center text-center space-y-6">
      ...
      <p className="text-gray-500 font-medium max-w-xs text-lg">Tu habitación ya está lista y el cronómetro ha comenzado.</p>
